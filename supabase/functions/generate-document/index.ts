      ...
      try {
        ...
        for (const [filename, bytes] of Object.entries(unzipped)) {
          ...
          xml = xml.replace(paraRe, (para) => {
            ...
            // 4) Canonicalize the GLYPH run: force Segoe UI Symbol + sz 20
            //    on the <w:rPr> of every <w:r> whose <w:t> body begins with
            //    a checkbox glyph. Ensures every checkbox renders identically
            //    in size and shape across every property block.
            const GLYPH_RPR =
              `<w:rFonts w:ascii="Segoe UI Symbol" w:hAnsi="Segoe UI Symbol" w:cs="Segoe UI Symbol"/>` +
              `<w:sz w:val="20"/><w:szCs w:val="20"/>`;
            newPara = newPara.replace(
              /<w:r\b([^>]*)>([\s\S]*?)<\/w:r>/g,
              (m, _rAttrs: string, body: string) => {
                // Only touch runs whose first <w:t> body starts with a glyph.
                const tMatch = body.match(/<w:t(?:\s[^>]*)?>([\s\S]*?)<\/w:t>/);
                if (!tMatch) return m;
                const txt = tMatch[1];
                if (!/^[\u2610\u2611\u2612]/.test(txt)) return m;
                let newBody: string;
                if (/<w:rPr>[\s\S]*?<\/w:rPr>/.test(body)) {
                  newBody = body.replace(
                    ...
                    (_mm, inner: string) => {
                      let cleaned = inner
                        ...
                        .replace(/<w:sz\b[^/]*\/>/g, "")
                        .replace(/<w:szCs\b[^/]*\/>/g, "");
                      return `<w:rPr>${GLYPH_RPR}${cleaned}</w:rPr>`;
                    },
                  );
                } else {
                  newBody = `<w:rPr>${GLYPH_RPR}</w:rPr>` + body;
                }
                return m.replace(body, newBody);
              },
            );

            // 5) Canonicalize the LABEL run: force Arial + sz 20 on the
            //    <w:rPr> of every <w:r> whose <w:t> body begins with one of
            //    the 7 PROPERTY TYPE labels. Keeps b/i/color if present.
            const LABEL_RPR =
            ...
            newPara = newPara.replace(
              ...
              (m, _rAttrs: string, body: string) => {
                const tMatch = body.match(/<w:t(?:\s[^>]*)?>([\s\S]*?)<\/w:t>/);
                if (!tMatch) return m;
                const txt = tMatch[1].replace(/^[\s\u00A0]+/, "");
                if (!labelStartRe.test(txt)) return m;
                let newBody: string;
                if (/<w:rPr>[\s\S]*?<\/w:rPr>/.test(body)) {
                  newBody = body.replace(
                    /<w:rPr>([\s\S]*?)<\/w:rPr>/,
                    (_mm, inner: string) => {
                      let cleaned = inner
                        ...
                        .replace(/<w:sz\b[^/]*\/>/g, "")
                        .replace(/<w:szCs\b[^/]*\/>/g, "");
                      return `<w:rPr>${LABEL_RPR}${cleaned}</w:rPr>`;
                    },
                  );
                } else {
                  newBody = `<w:rPr>${LABEL_RPR}</w:rPr>` + body;
                }
                return m.replace(body, newBody);
              },
            );

            // 6) Paragraph spacing + indent reset: drop <w:ind>, drop <w:tabs>,
            //    and force a fixed <w:spacing> so every row is the same height.
            //    Preserves <w:jc> and <w:numPr>.
            const FIXED_SPACING =
            ...
            newPara = newPara.replace(
              ...
              (_mm, inner: string) => {
                let cleaned = inner
                  ...
                  .replace(/<w:spacing\b[^>]*>[\s\S]*?<\/w:spacing>/g, "");
                return `<w:pPr>${cleaned}${FIXED_SPACING}</w:pPr>`;
              },
            );
          ...
          // 7) Containing table — fixed column widths. For every <w:tbl>
          //    that contains at least one matched PROPERTY TYPE paragraph,
          //    force a 9360 DXA, 2x4680 grid and matching cell widths.
          const TBL_W = `<w:tblW w:w="9360" w:type="dxa"/>`;
          const TBL_GRID = `<w:tblGrid><w:gridCol w:w="4680"/><w:gridCol w:w="4680"/></w:tblGrid>`;
          const TC_W = `<w:tcW w:w="4680" w:type="dxa"/>`;
          const TC_MAR = `<w:tcMar><w:top w:w="0" w:type="dxa"/><w:left w:w="120" w:type="dxa"/><w:bottom w:w="0" w:type="dxa"/><w:right w:w="120" w:type="dxa"/></w:tcMar>`;
          const tblRe = /<w:tbl\b[^>]*>[\s\S]*?<\/w:tbl>/g;
          const propLabelGate = new RegExp(
            `[\\u2610\\u2611\\u2612][\\s\\S]{0,200}?(?:${LABEL_PATTERN})(?![A-Za-z])`,
            "i",
          );
          xml = xml.replace(tblRe, (tbl) => {
            if (!propLabelGate.test(tbl)) return tbl;
            let newTbl = tbl;
            // Force <w:tblW>
            if (/<w:tblW\b[^/]*\/>/.test(newTbl)) {
              newTbl = newTbl.replace(/<w:tblW\b[^/]*\/>/, TBL_W);
            } else if (/<w:tblPr>([\s\S]*?)<\/w:tblPr>/.test(newTbl)) {
              newTbl = newTbl.replace(
                /<w:tblPr>([\s\S]*?)<\/w:tblPr>/,
                (_m, inner: string) => `<w:tblPr>${inner}${TBL_W}</w:tblPr>`,
              );
            }
            // Force <w:tblGrid>
            if (/<w:tblGrid>[\s\S]*?<\/w:tblGrid>/.test(newTbl)) {
              newTbl = newTbl.replace(
                /<w:tblGrid>[\s\S]*?<\/w:tblGrid>/,
                TBL_GRID,
              );
            }
            // Force per-cell <w:tcW> and <w:tcMar>
            newTbl = newTbl.replace(
              /<w:tcPr>([\s\S]*?)<\/w:tcPr>/g,
              (_m, inner: string) => {
                let cleaned = inner
                  ...
                  .replace(/<w:tcMar\b[^/]*\/>/g, "");
                return `<w:tcPr>${TC_W}${TC_MAR}${cleaned}</w:tcPr>`;
              },
            );
            if (newTbl !== tbl) count++;
            return newTbl;
