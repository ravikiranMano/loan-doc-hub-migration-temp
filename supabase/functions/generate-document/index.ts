    ...
    if (/851d/i.test(template.name || "")) {
      try {
        ...
        console.error(
          ...
          //   blank/other              → both ☐
          // Bounded look-ahead (4 KB) within each PROPERTY #K region. Skips
          // any glyph already overlapping a queued rewrite span so this never
          // collides with the merge-tag publisher when the template uses
          // {{pr_pt_actual_K_glyph}} / {{pr_pt_estimated_K_glyph}} directly.
          if (regions.props.length > 0) {
