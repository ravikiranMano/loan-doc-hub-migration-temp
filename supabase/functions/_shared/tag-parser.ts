...
export function replaceMergeTags(
  ...
  // When a template references lender data but does NOT contain
  // {{#each lenders}} or {{#each additionalLenders}} repeater tags, the
  // template author has no surface to render lenders 2..N. To prevent
  // additional lenders from silently disappearing from the generated
  // document, append a minimal signature block for each additional lender
  // immediately before </w:body>. Templates that DO have repeater tags
  // are skipped — the author controls formatting there. Templates that
  // never reference lender fields at all are also skipped so unrelated
  // documents (HUD-1, etc.) are unaffected.
  try {
    const originalHasRepeater =
      content.indexOf("{{#each lenders}}") !== -1 ||
      content.indexOf("{{#each additionalLenders}}") !== -1;
    const originalReferencesLender =
      /\{\{[^}]*(?:ld_p_|lender[._]|lenders\d|additionalLenders\d|lender_\d)/i.test(content) ||
      /«[^»]*(?:ld_p_|lender[._]|Lender_Name)/i.test(content);
    ...
    if (!originalHasRepeater && originalReferencesLender) {
      ...
      for (let n = 2; n <= 50; n++) {
        const existsRow =
          fieldValues.get(`lender_${n}_exists`)
          ?? fieldValues.get(`additionalLenders${n - 1}.exists`);
        if (!existsRow || String(existsRow.rawValue).toLowerCase() !== "true") {
          break;
        }
        const displayName =
      ...
      if (addl.length > 0 && result.indexOf("</w:body>") !== -1) {
        // Detect dominant font / size from the existing document so the
        // appended blocks visually match the rest of the template.
        const fontCounts: Record<string, number> = {};
        const fontMatches = result.match(/w:ascii="([^"]+)"/g) || [];
        for (const m of fontMatches) {
          const f = m.match(/w:ascii="([^"]+)"/)?.[1];
          if (f) fontCounts[f] = (fontCounts[f] || 0) + 1;
        }
        const fontFamily =
          Object.entries(fontCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || "Arial";
        const szCounts: Record<string, number> = {};
        const szMatches = result.match(/w:sz w:val="(\d+)"/g) || [];
        for (const m of szMatches) {
          const s = m.match(/w:val="(\d+)"/)?.[1];
          if (s) szCounts[s] = (szCounts[s] || 0) + 1;
        }
        const fontSize =
          Object.entries(szCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || "18";
        const fontSizeBold = String(Number(fontSize) + 2);
        ...
        const rPr =
          `<w:rPr><w:rFonts w:ascii="${fontFamily}" w:cs="${fontFamily}" w:eastAsia="${fontFamily}" w:hAnsi="${fontFamily}"/>` +
          `<w:color w:val="000000"/><w:sz w:val="${fontSize}"/><w:szCs w:val="${fontSize}"/><w:rtl w:val="0"/></w:rPr>`;
        const rPrBold =
          `<w:rPr><w:rFonts w:ascii="${fontFamily}" w:cs="${fontFamily}" w:eastAsia="${fontFamily}" w:hAnsi="${fontFamily}"/>` +
          `<w:b w:val="1"/><w:color w:val="000000"/><w:sz w:val="${fontSizeBold}"/><w:szCs w:val="${fontSizeBold}"/><w:rtl w:val="0"/></w:rPr>`;
        const pPr =
          `<w:pPr><w:spacing w:after="0" w:before="0" w:line="240" w:lineRule="auto"/>` +
          `<w:ind w:left="360" w:right="0" w:firstLine="0"/><w:jc w:val="left"/></w:pPr>`;
        const pPrSpaced =
          `<w:pPr><w:spacing w:after="0" w:before="120" w:line="240" w:lineRule="auto"/>` +
          `<w:ind w:left="360" w:right="0" w:firstLine="0"/><w:jc w:val="left"/></w:pPr>`;
        const hrPara =
          `<w:p><w:pPr><w:spacing w:after="0" w:before="200" w:line="240" w:lineRule="auto"/>` +
          `<w:ind w:left="360" w:right="0" w:firstLine="0"/>` +
          `<w:pBdr><w:bottom w:val="single" w:sz="4" w:space="1" w:color="000000"/></w:pBdr></w:pPr>` +
          `<w:r><w:rPr><w:rtl w:val="0"/></w:rPr><w:t xml:space="preserve"> </w:t></w:r></w:p>`;
        const emptyPara =
          `<w:p>${pPr}<w:r>${rPr}<w:t xml:space="preserve"> </w:t></w:r></w:p>`;
        const textPara = (text: string, props = pPr, runProps = rPr) =>
          `<w:p>${props}<w:r>${runProps}<w:t xml:space="preserve">${esc(text)}</w:t></w:r></w:p>`;
    ...
    console.warn(`[MultiLender] Failed to append additional lender blocks: ${(err as Error).message}`);
  }
