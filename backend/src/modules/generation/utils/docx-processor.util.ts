...
import type { DocxProcessingOptions, FieldValueData, LabelMapping } from "./types";
import { replaceMergeTags } from "./tag-parser.util";
 ...
 *     modern DrawingML in mc:AlternateContent â€” Word renders mc:Choice).
 ...
 *
 * This is a localized string edit on the root opening tag only â€” it does
...
export async function processDocx(
  ...
  // Detailed per-stage timing â€” printed at end so production logs always
      ...
      if (isContentPart) {
        ...
        if (filename === "word/document.xml") {
          ...
          if (hasExistingPageBreaks) {
            ...
            debugLog("[docx-processor] Skipping signature page-break injection (single-page template â€” no existing page breaks).");
          }
        }
 ...
 * file is uploaded â€” otherwise mutations made after processDocx's internal
  ...
  while ((tagMatch = tagRe.exec(trimmed)) !== null) {
    ...
    if (body.startsWith("/")) {
      ...
      if (openName !== closeName) {
        ...
        console.error(`[docx-processor] DOCX_INTEGRITY well-formed failure in ${partName} at offset ${off} (stack top expected </${openName || "none"}>); ctx=â€¦${ctx}â€¦`);
      ...
      if (firstSuspect !== -1) {
        ...
        console.error(
          `[docx-processor] DOCX_INTEGRITY context for <${tag}> in ${partName} ` +
            `at offset ${firstSuspect}: â€¦${xml.slice(sliceStart, sliceEnd)}â€¦`,
        );
      }
      throw new Error(
 ...
 * pop the stack. Strictly conservative â€” only acts on a w:rPr / non-w:rPr
  ...
  while ((m = tagRe.exec(xml)) !== null) {
    ...
    if (body.startsWith("/")) {
      ...
      if (stack.length > 0 && stack[stack.length - 1].name === closeName) {
        ...
        // Unbalanced in some other way â€” bail out without modifying.
  ...
  while ((m = tagRe.exec(xml)) !== null) {
    ...
    if (body.startsWith("/")) {
      ...
      if (stack.length > 0 && stack[stack.length - 1].name === closeName) {
        ...
        // Unbalanced in some other way â€” bail without modifying.
        return { xml, repaired: 0 };
      }
    } else if (!body.endsWith("/")) {
      const name = body.split(/\s+/)[0];
