...
import type { ParsedMergeTag, FieldValueData, LabelMapping } from "./types";
import { applyTransform, formatByDataType, formatCheckbox } from "./formatting.util";
import { resolveFieldKeyWithMap, getFieldData } from "./field-resolver.util";
...
/**
 * Flatten Word MERGEFIELD structures into plain Â«fieldNameÂ» text runs.
 * 
 ...
 * 
 * This function converts both patterns into a single <w:r> with Â«fieldNameÂ» text,
...
function flattenMergeFieldStructures(xml: string): string {
  ...
  const result = processParaByPara(xml, (para) => {
    ...
    // Pattern C: Field codes WITHOUT a `separate` marker: begin â†’ instrText â†’ end.
...
export function normalizeWordXml(xmlContent: string, templateName = ""): string {
  ...
  // First: flatten Word MERGEFIELD structures into plain Â«fieldNameÂ» text runs
  ...
  // detector (`\{(?:\s|<[^>]+>)+\{` / `\}(?:\s|<[^>]+>)+\}`) â€” which can
  // match intact-but-adjacent tags in dense templates â€” we can verify
  ...
  if (xmlContent.length > 200_000 && hasFragmentedCandidates && !hasFieldCodeStructures) {
    ...
    if (!anyParaFragmented) {
      console.log(
        `[tag-parser] normalizeWordXml fast-path: large template with no per-paragraph fragmentation â€” skipped heavy normalization (${xmlContent.length}B, total=${Date.now() - __nwxStart}ms)`
      );
      return result;
    }
  }
  ...
  // Handle fragmented merge fields, braces, conditionals â€” all paragraph-scoped
  ...
  result = processParaByPara(result, (para) => {
    ...
    p = p.replace(fragmentedPattern, (match, pre, fieldName, post) => {
      ...
      return `Â«${fieldName}Â»`;
    });

    // Handle XML-fragmented chevron patterns
    const leftChevronFragmented = /Â«((?:\s*<\/w:t>\s*<\/w:r>\s*<w:r(?:[^>]*)>\s*<w:t(?:[^>]*)>)+)/g;
    p = p.replace(leftChevronFragmented, () => "Â«");

    const rightChevronFragmented = /((?:\s*<\/w:t>\s*<\/w:r>\s*<w:r(?:[^>]*)>\s*<w:t(?:[^>]*)>)+)Â»/g;
    p = p.replace(rightChevronFragmented, () => "Â»");
    ...
    //   {{#if KEY}}â˜‘{{else}}â˜{{/if}}  (or any combination of â˜‘ / â˜’ / â˜)
    ...
    p = p.replace(chevronFragmented, (match, inner) => {
      ...
      if (/^[A-Za-z0-9_.]+$/.test(cleanText)) {
        if (inner.includes('<')) {
          debugLog(`[tag-parser] Consolidated fragmented chevron tag: Â«${cleanText}Â»`);
        }
        return `Â«${cleanText}Â»`;
      }
      return match;
    });
...
export function consolidateAppraiserConditional(xml: string): string {
  ...
  return processParaByPara(xml, (para) => {
    ...
    // (Cannot guard on contiguous payload text â€” `N/A` is intentionally split
...
function consolidateFragmentedTagsInParagraphs(xml: string): string {
  ...
  const result = processParaByPara(xml, (para) => {
    ...
    if (tTexts.length < 2) return para; // Single or no text runs â€” no cross-run fragmentation
    ...
    // Tags are fragmented across runs â€” proceed to consolidation
    parasConsolidated++;
 ...
 * Supports: Â«field_nameÂ», {{field_name}}, {{field_name|transform}}, MERGEFIELD instructions
 */
export function parseWordMergeFields(content: string): ParsedMergeTag[] {
  ...
  if (content.includes('{{') && content.includes('}}')) {
    ...
    while ((match = curlyPattern.exec(content)) !== null) {
      const tagName = match[1].trim();
      // Skip conditional/control tags â€” these are handled by processConditionalBlocks
  ...
  if (content.includes('MERGEFIELD')) {
    ...
    while ((match = mergeFieldPattern.exec(content)) !== null) {
      ...
      const syntheticTag = `Â«${fieldName}Â»`;
 ...
 * This produces an interactive, clickable checkbox in Word â€” matching the
...
function buildSdtCheckboxXml(isChecked: boolean, rPr?: string): string {
  ...
  const displayChar = isChecked ? '\u2611' : '\u2610'; // â˜‘ or â˜
  ...
  // Gothic). Without this, glyphs constructed from static template â˜/â˜‘
  // runs render visibly smaller than the template's native SDT checkboxes
  // because:
  //   - the body default font (Arial/Times) provides a thinner â˜/â˜‘ glyph
  ...
  if (rPr) {
    ...
    // inline {{#if}}â˜’{{else}}â˜{{/if}} placeholders authored at 9-10pt in the
 ...
 * checkbox glyph (â˜, â˜‘, or â˜’) into a native Word SDT checkbox.
 * This runs after all merge-tag and label-based replacements are complete,
 * so every checkbox glyph â€” regardless of origin â€” becomes interactive.
 */
function convertGlyphsToSdtCheckboxes(xml: string): string {
  if (xml.indexOf('â˜') === -1 && xml.indexOf('â˜‘') === -1 && xml.indexOf('â˜’') === -1) {
    return xml;
  }
  ...
  const runWithGlyphOnly = /<w:r\b[^>]*>\s*(?:<w:rPr>((?:(?!<\/w:rPr>|<w:r\b|<w:p\b|<w:tbl\b)[\s\S])*?)<\/w:rPr>)?\s*<w:t[^>]*>([â˜â˜‘â˜’])<\/w:t>\s*<\/w:r>/g;
  ...
  safeguardedXml = safeguardedXml.replace(runWithGlyphOnly, (_match, innerRPr, glyph) => {
    const isChecked = glyph === 'â˜‘' || glyph === 'â˜’';
    count++;
    return buildSdtCheckboxXml(isChecked, innerRPr || undefined);
  });

  // Restore SDT blocks (reverse order to handle nesting correctly â€” inner restored first)
...
export function isInsideTextRun(xml: string, index: number): boolean {
  ...
  // Must be <w:t> or <w:t ...> â€” not <w:tbl>, <w:tr>, <w:tc>, <w:tab/>, etc.
...
function replaceStaticCheckboxLabel(
  ...
  // failures. Removing the redundant pass changes no output bytes â€” the
  // global dedup still runs once and produces the identical result.
  const glyphInWtPattern = new RegExp(
    `(<w:t[^>]*>)([^<]*?)([â˜â˜‘â˜’])([^<]*?</w:t>)((?:\\s|<[^>]+>)*?${labelPattern})(?![A-Za-z])`,
    'gi'
  );
  ...
  // (e.g. "Lender(s) as Additional Insuredâ˜‘") rather than before it.
  const trailingGlyphPattern = new RegExp(
    `(${labelPattern})((?:\\s|<[^>]+>)*)([â˜â˜‘â˜’])`,
    'gi'
  );
  ...
  const plainPattern = new RegExp(`([â˜â˜‘â˜’])((?:\\s|<[^>]+>)*)(${labelPattern})(?![A-Za-z])`, 'gi');
...
// adjacent â˜/â˜‘/â˜’ glyphs that arise when both a merge tag AND a static
...
export function replaceLabelBasedFields(
  ...
  const processedContent = processParaByPara(content, (segment) => {
    ...
    const hasCheckboxGlyph =
      segment.indexOf("â˜") !== -1 || segment.indexOf("â˜‘") !== -1 || segment.indexOf("â˜’") !== -1;
    ...
    for (const { label, mapping, quickNeedle, fieldKeyLower } of enrichedCandidates) {
      ...
      // Strip leading "$" from currency values â€” the template already contains a literal "$" prefix
...
function getConditionalAliasCandidates(fieldKey: string): string[] {
  ...
  // Subordination Provision â€” RE851A Yes/No checkbox.
  ...
  // "Is Broker Also a Borrower?" â€” RE851A Part 2 A/B capacity checkboxes.
  ...
  // aliases. Order matters â€” the first key that has saved deal data wins.
 ...
 *   - For mixed boolean â†” string compares, both sides are normalized to the
...
function evaluateEqExpression(
  ...
  // (eq FIELD LITERAL) â€” LITERAL is bareword OR "..." OR '...'
...
/**
 * Split the children of an (or â€¦) / (and â€¦) sexp into balanced sub-expressions.
 ...
 * WITHOUT their outer parens â€” e.g. `["eq A \"x\"", "eq B \"y\""]`. Bare-word
 ...
 *   - or  (sexp1) (sexp2) â€¦                             (any true â†’ true)
 *   - and (sexp1) (sexp2) â€¦                             (all true â†’ true)
...
function evaluateSubExpression(
  ...
  if (headMatch) {
    ...
    for (const child of children) {
      // Bare-word child â†’ truthy check on the field value
      let v: boolean | null;
...
/**
 * Find the first `{{#if (â€¦)}}` or `{{#unless (â€¦)}}` opener whose parens are
...
function findBalancedSexpBlock(
  ...
  while ((m = openerRe.exec(source)) !== null) {
    ...
    // Find matching {{/if}} or {{/unless}} (innermost â€” track nesting of same tag)
    const closeTag = `{{/${tag}}}`;
...
function isConditionTruthy(
  ...
  // Always probe aliases â€” if any alias key actually has saved deal data,
  ...
  if (resolved?.data) {
    ...
    if (typeof raw === "string") {
      ...
      // Unknown non-empty string â€” treat as truthy (legacy behavior).
      return true;
    }
...
export function processConditionalBlocks(
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    // handful of conditionals â€” without this guard, each iteration still
    ...
    // Pre-pre-pass: resolve any {{#if (â€¦)}} / {{#unless (â€¦)}} whose head is
    // (or â€¦), (and â€¦), (not â€¦), or a nested combination thereof. The simple
    // (eq â€¦) pattern below cannot match these, and leaving them unresolved
    ...
    if (sexpBlock) {
      ...
      // Only rewrite when we successfully evaluated; otherwise let the (eq â€¦)
    ...
    // {{#if (eq ...)}}â€¦{{/if}} block. We rewrite it to either the true or
    // else branch so the simple {{#if KEY}} matcher below sees clean content.
    // Same for {{#unless (eq ...)}}.
    // Cheap pre-check: only run the (eq â€¦) regexes when an `(eq ` literal is
    ...
    // Literal forms accepted: &quot;â€¦&quot;, "â€¦", 'â€¦', smart-quoted â€œâ€¦â€/â€˜â€¦â€™,
    ...
    if (ifMatch && (!unlessMatch || ifMatch.index < unlessMatch.index)) {
      ...
      if (truthy) {
        ...
        if (elseIndex !== -1) {
          ...
          // No else branch â€” remove block and clean up surrounding empty paragraphs
  ...
  if (iterations >= MAX_ITERATIONS) {
    console.warn("[tag-parser] Hit max iterations for conditional processing â€” possible malformed tags");
  }
  ...
  // (e.g. `{{#ifpr_li_ant_balloonYes_N_S}}`) â€” strip the opener so it does
  ...
  // We only remove text inside <w:t>â€¦</w:t> runs whose entire content matches
...
export function processSdtCheckboxes(
  ...
  // Match entire <w:sdt>â€¦</w:sdt> blocks that contain a w14:checkbox element
  ...
  return content.replace(sdtPattern, (sdtBlock, offset) => {
    ...
    if (tagMatch) {
      // Tagged SDT â€” resolve via tag name
      const tagName = tagMatch[1];
      canonicalKey = resolveFieldKeyWithMap(tagName, mergeTagMap, validFieldKeys);
    } else if (labelMap) {
      // Tagless SDT â€” look at surrounding text in the same table row or paragraph
      ...
      if (!canonicalKey) {
        debugLog("[tag-parser] SDT checkbox found but no <w:tag> and no label match â€” skipping");
        return sdtBlock;
      }
    } else {
      debugLog("[tag-parser] SDT checkbox found but no <w:tag> â€” skipping");
      return sdtBlock;
    }

    const resolved = getFieldData(canonicalKey, fieldValues);

    // Determine boolean state â€” default to unchecked when no data exists
    let isChecked = false;
    if (resolved?.data) {
      const raw = resolved.data.rawValue;
      if (typeof raw === "string") {
...
export function processEachBlocks(
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    if (sortedIndices.length === 0) {
      // No entities found â€” remove the entire block
      ...
      for (const idx of sortedIndices) {
        ...
        // Find all merge tags within the block: {{fieldName}}, {{fieldName|transform}}, Â«fieldNameÂ»
        const innerCurlyTags = [...blockContent.matchAll(/\{\{([^}#/|]+)(?:\|([^}]+))?\}\}/g)];
        const innerChevronTags = [...blockContent.matchAll(/Â«([^Â»]+)Â»/g)];
        ...
        for (const tag of innerCurlyTags) {
          ...
          // Handlebars-style {{this.field}} â†’ resolve as {{field}} on current entity.
        ...
        //   {{#unless @last}}â€¦{{/unless}}     â€” emit when not the final clone
        //   {{#if @last}}â€¦{{/if}}             â€” emit only on the final clone
        //   {{#unless @first}}â€¦{{/unless}}    â€” emit when not the first clone
        //   {{#if @first}}â€¦{{/if}}            â€” emit only on the first clone
        //   {{@index}}                         â€” 0-based iteration index
        ...
        // surrounding `{{#unless â€¦}}` blocks untouched.
        ...
        // Bare {{@index}} (no helper) â†’ numeric value.
        ...
        // bare identifiers â€” keys that already contain a "." (or already
  ...
  if (iterations >= MAX_ITERATIONS) {
    console.warn("[tag-parser] Hit max iterations for {{#each}} processing â€” stripping any remaining unmatched block markers to keep XML valid");
  }
...
export function replaceMergeTags(
  ...
  // cost to push the Edge Function past its 2s isolate budget â€” surfacing as
  ...
  const contentLower = content.toLowerCase();
  const hasMergeMarkers = content.includes('{{') || content.includes('}}') || content.includes('Â«') || content.includes('Â»') || content.includes('MERGEFIELD') || content.includes('w:fldChar') || content.includes('w:fldSimple') || content.includes('w:instrText');
  ...
  // between recognized control tokens â€” surrounding prose is never touched.
  //
  ...
  if (result.indexOf('{{lender_') !== -1) {
    ...
    if (lenderCount === 0) {
      console.warn('[tag-parser] Template references {{lender_N_*}} tags but lender_count is 0 â€” stripping unresolved tags');
    }
    result = result.replace(
  ...
  // markers ("[ ]", "[x]", "[X]") instead of checkbox glyphs (â˜/â˜‘) next to
  ...
  // glyph here. The replacement is strictly bounded to the marker region â€”
    ...
    for (const label of booleanLabels) {
      ...
      result = result.replace(
        ...
        (_m, wtOpen, pre, mark, wtTail, labelPart) => {
          const glyph = mark ? 'â˜‘' : 'â˜';
          return `${wtOpen}${pre}${glyph}${wtTail}${labelPart}`;
        }
      );
      // Plain-text fallback (no <w:t> wrapper) â€” same scoping, marker only.
  ...
  // engine to attempt each alternative at every position â€” a major CPU sink.
  ...
  if (tagReplacementMap.size > 0) {
    ...
    // glyphs that arise when a merge tag resolves to â˜‘/â˜ next to a static â˜
    // already present in the template. Skip entirely when no glyphs exist
    // anywhere in the document â€” the lazy regex scan is otherwise O(N) over
  ...
  // and had no data. Do NOT globally remove all {{...}} patterns â€” that can
  ...
  // result â€” the combined-regex pass above already replaced every parsed tag
  ...
  if (/<w:t[^>]*>\s*<\/w:t>/.test(result)) {
    ...
    result = processParaByPara(result, (para) => {
      ...
      // Paragraph has no visible text â€” but only remove it if it previously
  ...
  // Clean up orphaned }} that remain as literal text after tag replacement.
  // Mirror of the orphaned {{ cleanup above â€” handles cases where Word splits
  ...
  // RE851A â€” Subordination Provision Yes/No safety pass.
  ...
  // Supports both static glyphs (â˜/â˜‘/â˜’) and native Word SDT checkboxes
  ...
  if (is851A) {
    ...
    if (isSubordination !== null) {
      const yesGlyph = isSubordination ? "â˜‘" : "â˜";
      const noGlyph = isSubordination ? "â˜" : "â˜‘";
      ...
      // with a literal â˜‘ glyph typed inside <w:rPr> of <w:sdt>/<w:sdtContent>
      ...
      // producing phantom "â˜‘â˜‘" prefixes that no toggle can affect because
      // they live OUTSIDE <w:t>. Removing only the bare text â€” never tags â€”
      ...
      const toggleSdtBlock = (sdtBlock: string, isChecked: boolean): string => {
        ...
        if (selfClosing.test(updated)) {
          ...
          // <w14:checked> missing â€” inject it as the first child of the
      ...
      // checkboxes to be replaced by a single toggled block â€” leaving the
      ...
      // (Yes/No) â€” across XML/whitespace only â€” to the desired state.
      ...
      // (Yes/No) â€” across XML/whitespace only, never crossing a paragraph.
      ...
      // Force a static glyph (â˜/â˜‘/â˜’) immediately before the word.
      const forceGlyphBeforeWord = (
        ...
        // 0) Glyph and label inside the SAME <w:t> run (e.g. "â˜ Yes").
        const glyphSameWt = new RegExp(
          `(<w:t[^>]*>)([^<]*?)([â˜â˜‘â˜’])(\\s*${wordRe}[^<]*</w:t>)`,
          "g",
        );
        ...
        const glyphInWt = new RegExp(
          `(<w:t[^>]*>)([^<]*?)([â˜â˜‘â˜’])([^<]*?</w:t>)(${inlineXmlOnly}${wordRe})`,
          "g",
        );
        next = next.replace(
          glyphInWt,
      ...
      // Force a static glyph (â˜/â˜‘/â˜’) immediately after the word.
      const forceGlyphAfterWord = (
        ...
        const inlineXmlOnly = `(?:\\s|<(?!w:sdt\\b|\\/w:sdt\\b|\\/w:p\\b|w:p[\\s>\\/])[^>]+>)*?`;
        const glyphInWt = new RegExp(
          `(${wordRe})(${inlineXmlOnly}<w:t[^>]*>)([^<]*?)([â˜â˜‘â˜’])([^<]*?</w:t>)`,
          "g",
        );
        let next = windowXml.replace(
          glyphInWt,
      ...
      // paragraph or column/page (RE851A v5 places them ~5â€“6KB after the
      ...
      while ((anchorMatch = anchorRe.exec(result)) !== null) {
        ...
        // imprecise vs XML, but acceptable here â€” markers are unique).
        ...
        // Native Word SDT checkboxes â€” both label orientations.
        ...
        // Static glyphs â€” both label orientations.
        ...
        // Final-final pass â€” paragraph-scoped Yes/No glyph normalization.
        // Some RE851A authoring leaves the Subordination Provision row using
        // clean Handlebars text checkboxes ({{#if KEY}}â˜‘{{else}}â˜{{/if}} Yes)
        ...
        // glyphs (â˜‘ AND â˜) present before the literal "Yes" or "No". The
        ...
        // strictly to this row â€” no other paragraph is touched.
        ...
        windowXml = windowXml.replace(
          ...
          (para) => {
            ...
            const hasYes = /(?:[â˜â˜‘â˜’]|\}\})\s*Yes\b/.test(trimmed)
              || /(?:^|\s)Yes\b/.test(trimmed);
            const hasNo = /(?:[â˜â˜‘â˜’]|\}\})\s*No\b/.test(trimmed)
              || /(?:^|\s)No\b/.test(trimmed);
            ...
            const hasCheckboxArtifact = /[â˜â˜‘â˜’]/.test(trimmed)
            ...
            // Skip paragraphs that already use native Word SDT checkboxes â€”
            ...
            p = p.replace(
              /<w:t([^>]*)>([^<]*)<\/w:t>/g,
              (m, attrs, text) => {
                if (!/[â˜â˜‘â˜’]/.test(text)) return m;
                const cleaned = text.replace(/[â˜â˜‘â˜’]/g, "");
                return `<w:t${attrs}>${cleaned}</w:t>`;
              },
            );
        ...
        // Final pass â€” RE851A live layout: a paragraph containing only the
        ...
        windowXml = windowXml.replace(
          ...
          (para) => {
            if (/<w:sdt\b/.test(para)) return para;
            if (/[â˜â˜‘â˜’]/.test(para)) return para;
  ...
  // RE851A Part 3 â€” Amortization (CHECK ONE) safety pass.
  //
  ...
  //   (a) a checkbox glyph (â˜/â˜‘/â˜’) inside the most recent <w:t> before the
  //       label, OR
  //   (b) the most recent native <w:sdt><w14:checkbox>â€¦</w:sdt> block before
  ...
  if (is851A) {
    ...
    if (anyDerived) {
      ...
      // Scoped to a single <w:sdt>â€¦</w:sdt> block â€” never touches sibling
      ...
      const updateSdtBlock = (sdt: string, isChecked: boolean): string => {
        ...
        next = next.replace(
          /(<w:sdtContent\b[\s\S]*?<w:t[^>]*>)[â˜â˜‘â˜’](<\/w:t>[\s\S]*?<\/w:sdtContent>)/,
          `$1${glyphChar}$2`,
        );
        return next;
      };
      ...
      const forceCheckboxForLabel = (xml: string, label: string, isChecked: boolean): string => {
        ...
        //    Only the glyph character is rewritten â€” surrounding text and
        //    formatting runs are untouched.
        const glyphInWt = new RegExp(
          `(<w:t[^>]*>)([^<]*?)([â˜â˜‘â˜’])([^<]*?</w:t>)((?:\\s|<[^>]+>)*?${labelPattern})${guard}`,
          "gi",
        );
      ...
      // before "AMORTIZED" â€” combined with the negative lookahead in
  ...
  // RE851A â€” Payable (Monthly / Annually) safety pass.
  ...
  //   Monthly   â†’ â˜‘ "Monthly",  â˜ "Annually"
  //   Annually  â†’ â˜ "Monthly",  â˜‘ "Annually"
  ...
  // preserved unchanged â€” only the glyph character toggles.
  if (is851A) {
    const payableRaw =
    ...
    if (payableMode !== null) {
      const monthlyGlyph  = payableMode === "monthly"  ? "â˜‘" : "â˜";
      const annuallyGlyph = payableMode === "annually" ? "â˜‘" : "â˜";
  ...
  // RE851A Part 2 â€” broker-capacity A/B paragraph-split normalization.
  //
  ...
  // mode is that Option B's checkbox is missing or never flips to â˜‘ when
  ...
  //
  // Strictly scoped to this single A./B. row â€” no other paragraphs or
  // sections of the document are touched.
  if (is851A) {
    const splitParaRe = /<w:p\b([^>]*)>([\s\S]*?)<\/w:p>/g;
    result = result.replace(splitParaRe, (full, pAttrs, inner) => {
      // Plain text of the paragraph â€” used to detect both labels are present.
      ...
      //   â€¢ Spacing Before = 0 pt
      //   â€¢ Spacing After  = 0 pt
      //   â€¢ Line spacing   = Single (240 twentieths of a point, auto rule)
  ...
  // RE851A Part 2 â€” broker-capacity A/B safety pass.
  ...
  if (is851A) {
    ...
    if (isBrkBorrower !== null) {
      ...
      //   Yes â†’ A. Agent unchecked, B. Principal checked
      //   No  â†’ A. Agent checked,   B. Principal unchecked
      ...
      const aGlyph = isBrkBorrower ? "â˜" : "â˜‘"; // A. Agent (1st)
      const bGlyph = isBrkBorrower ? "â˜‘" : "â˜"; // B. Principal (2nd)
  ...
  // RE851A â€” Servicing Agent safety pass.
  ...
  //   Lender                    â†’ â˜‘ "THERE ARE NO SERVICING ARRANGEMENTS"
  //   Broker                    â†’ â˜‘ "BROKER IS THE SERVICING AGENT"
  //   Company / Other Servicer  â†’ â˜‘ "ANOTHER QUALIFIED PARTY WILL SERVICE THE LOAN"
  // All other servicing glyphs become â˜. This guarantees exactly one â˜‘
  // even when the template's {{#if (eq â€¦)}} blocks were broken across
  // <w:r>/<w:t>/SDT runs and left a static â˜ behind. Surrounding text,
  // formatting, and XML structure are preserved unchanged â€” only the
  ...
  if (is851A) {
    ...
    if (mode !== null) {
      const lenderGlyph = mode === "lender" ? "â˜‘" : "â˜";
      const brokerGlyph = mode === "broker" ? "â˜‘" : "â˜";
      const otherGlyph  = mode === "other"  ? "â˜‘" : "â˜";
  ...
  // Glyph standardization: replace any ballot-box-with-X (â˜’) with check
  // (â˜‘) so the "checked" state renders consistently across all generated
  // documents. Logic unchanged â€” display glyph only.
  if (result.indexOf('\u2612') !== -1) {
    result = result.replace(/\u2612/g, '\u2611');
  }

  // Final pass: convert any remaining checkbox glyphs (â˜/â˜‘) in <w:r>
