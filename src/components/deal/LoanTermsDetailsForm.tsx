...
const HOLD_REASON_OPTIONS = [
  ...
  { value: 'payment_issue', label: 'Pending Payoff' },
    ...
    <div className="p-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-6 gap-y-0">
        ...
        <div className="space-y-1.5">
          ...
          {renderInlineField('loan.project_number', 'Project Number')}
          ...
          {renderInlineDateField('loan.paid_off_date', 'Paid Off / Closed')}
        ...
        <div className="space-y-1.5">
          ...
          {renderInlineCheckbox('loan.type_section32', 'Section 32')}
          {renderInlineCheckbox('loan.type_article7', 'Article 7')}
          ...
          <DirtyFieldWrapper fieldKey="loan.type_on_pull">
            <div className="flex items-center space-x-2">
              <Checkbox
                id="loan.type_on_pull"
                checked={getBoolValue('loan.type_on_pull')}
                onCheckedChange={(c) => setBoolValue('loan.type_on_pull', !!c)}
                disabled={disabled}
                className="h-3.5 w-3.5"
              />
              <Label htmlFor="loan.type_on_pull" className="font-normal cursor-pointer text-xs text-green-600">On Pull</Label>
            </div>
          </DirtyFieldWrapper>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          {(() => {
            const status = getValue(FIELD_KEYS.loanStatus);
            return (
              <>
                ...
                {status === 'hold' && renderInlineSelect(FIELD_KEYS.holdReason, 'Hold Reason', HOLD_REASON_OPTIONS, 'Select')}
                {status === 'closed' && renderInlineSelect(FIELD_KEYS.closedReason, 'Closed Reason', CLOSED_REASON_OPTIONS, 'Select')}
              </>
            );
          })()}
          ...
          {/* Send block */}
          <div className="pt-3">
            <h4 className="font-semibold text-xs text-foreground mb-2">Send:</h4>
            {[
              ...
              const lastSentRaw = getValue(row.ls);
              const lastSentDisplay = (() => {
                const d = safeParseDateStr(lastSentRaw);
                return d ? format(d, 'MM/dd/yyyy') : lastSentRaw || '';
              })();
              return (
                <div key={row.cb} className="flex items-center gap-2 mb-1.5">
                  ...
                  <Input
                    ...
                    readOnly
                    disabled
                    placeholder="Last Sent"
                    className="h-8 text-xs w-[110px] bg-muted/40"
                  />
                </div>
              );
            })}
          ...
          <div className="pt-2 space-y-1.5">
            <div className="flex items-center gap-2">
              <Label className="flex-1 text-xs">NSF Previous 12 Months</Label>
              <Input
                value={getValue('loan.nsf_prev_12mo')}
                readOnly
                disabled
                className="h-8 text-xs w-[110px] bg-muted/40"
              />
            ...
            <div className="flex items-center gap-2">
              <Label className="flex-1 text-xs">30-days Plus</Label>
              <Input
                value={getValue('loan.thirty_days_plus')}
                readOnly
                disabled
                className="h-8 text-xs w-[110px] bg-muted/40"
              />
